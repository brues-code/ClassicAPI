...
| Retail-like `/reload` | `/reload` picks up files created while the game runs, like a modern client. A new folder under `Interface\AddOns\` registers and loads as a normal addon, new files added to an existing addon's TOC load, and a newly installed addon's first SavedVariables save survives `/reload` (a stock client needs a full restart for all three). `##` metadata edits to an already-registered addon still need a re-login. See [`/reload` picks up new addons and new files](docs/API.md#reload-picks-up-new-addons-and-new-files). |
