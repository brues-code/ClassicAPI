...
| Settings profiles | A `-config <name>` launch switch. The client reads and writes `WTF\<name>` in place of `WTF\Config.wtf`, so one install can hold several settings profiles. Add it to the launcher shortcut, for example `VanillaFixes.exe -config Config2.wtf`. The name must be a plain filename, with no folder in it. On first use the new file does not exist yet, so the client starts from its defaults and writes the file when you quit. |
| Startup scripts | Four launch switches that run Lua as the client starts. `-gluescript <code>` and `-gluescriptFile <path>` run at the login and character-select screens, every time those screens come up. `-gamescript <code>` and `-gamescriptFile <path>` run one time, after you enter the world. Quote code that contains spaces, for example `VanillaFixes.exe -gluescript "print(GetLocale())"`. A file path can be absolute, and the file is read when the script runs, so you can edit it between launches. Each script runs after the interface is loaded, so it can call interface functions and use frames. Errors are reported the same way any script error is. |
