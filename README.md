...
| [Upvalue limit](docs/API.md#upvalue-limit) | A function can use up to 255 upvalues, the outer locals it refers to. Lua 5.0 stops at 32 and rejects the whole file. Lua 5.1 allows 60, so a large handler that reads many file-level locals can be valid 5.1 and still fail to load here. Nothing changes for a function within 32. |
