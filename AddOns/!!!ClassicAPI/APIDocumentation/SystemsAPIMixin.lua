-- Backport of Blizzard_APIDocumentation/SystemsAPIMixin.lua to vanilla 1.12 /
-- Lua 5.0.
--
...
--   - A search also matches a system's namespace. The namespace is the name
--     people type, and retail's version checks only the short name, so
--     "search C_Spell" found nothing while "search spell" found the system.
...
function SystemsAPIMixin:MatchesSearchString(searchString)
	...
	local namespace = self:GetLoweredNamespaceName();
	if namespace ~= "" and namespace:match(searchString) then
		return true;
	end
...
function SystemsAPIMixin:GetDetailedOutputLines()
	...
	if self.Functions and tGetN(self.Functions) > 0 then
	...
	if self.Events and tGetN(self.Events) > 0 then
	...
	if self.Tables and tGetN(self.Tables) > 0 then
...
function SystemsAPIMixin:GetNumTables()
	return self.Tables and tGetN(self.Tables) or 0;
end

function SystemsAPIMixin:GetNumFunctions()
	return self.Functions and tGetN(self.Functions) or 0;
end

function SystemsAPIMixin:GetNumEvents()
	return self.Events and tGetN(self.Events) or 0;
end
